...
!(function(e) {
	e.fn.imageUploader = function(t) {
		...
		let o = function() {
				...
				e('<i>', { class: 'fas fa-cloud-upload-alt' }).appendTo(i),
			...
			g = function(e) {
				return (
					!(a.settings.extensions.indexOf(e.name.replace(new RegExp('^.*\\.'), '.')) < 0) ||
					(alert(
						`The file "${e.name}" does not match with the accepted file extensions: "${a.settings.extensions.join(
							'", "'
						)}"`
					),
					!1)
			...
			c = function(e) {
				return (
					...
					(alert(
						`The file "${e.name}" does not match with the accepted mime types: "${a.settings.mimes.join(
							'", "'
						)}"`
					),
					!1)
			...
			f = function(e) {
				return (
					...
					(alert(`The file "${e.name}" exceeds the maximum size of ${a.settings.maxSize / 1024 / 1024}Mb`),
					!1)
				);
			},
			m = function(e, t) {
				return (
					!(e + s.items.length + a.settings.preloaded.length >= a.settings.maxFiles) ||
					(alert(
						`The file "${t.name}" could not be added because the limit of ${a.settings
							.maxFiles} files was reached`
					),
					!1)
				);
			},
